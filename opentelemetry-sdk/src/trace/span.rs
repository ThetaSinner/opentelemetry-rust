...
use crate::Resource;
use ts_opentelemetry_api::trace::{Event, SpanContext, SpanId, SpanKind, Status};
use ts_opentelemetry_api::{trace, KeyValue};
use std::borrow::Cow;
...
impl ts_opentelemetry_api::trace::Span for Span {
...
impl Span {
    fn ensure_ended_and_exported(&mut self, timestamp: Option<SystemTime>) {
        ...
        if let Some(timestamp) = timestamp {
            ...
            data.end_time = ts_opentelemetry_api::time::now();
        }
...
mod tests {
    ...
    use ts_opentelemetry_api::trace::{Link, TraceFlags, TraceId, Tracer};
    use ts_opentelemetry_api::{trace::Span as _, trace::TracerProvider, KeyValue};
    ...
    fn add_event_with_timestamp() {
        ...
        let name = "some_event";
        let attributes = vec![KeyValue::new("k", "v")];
        let timestamp = ts_opentelemetry_api::time::now();
